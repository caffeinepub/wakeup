...
import {
  ...
  useGetJobPostingsByEmployer,
  useToggleJobPostingActive,
...
function JobApplications({ jobId }: { jobId: bigint }) {
  const {
    ...
    isLoading,
    isError,
  } = useGetApplicationsForJob(jobId);
      ...
      <CardContent className="p-5">
        ...
        <Collapsible open={applicationsOpen} onOpenChange={setApplicationsOpen}>
          <CollapsibleTrigger asChild>
            <button
              type="button"
              className="mt-3 w-full flex items-center justify-between px-3 py-2 rounded-lg bg-secondary hover:bg-secondary/80 transition-colors text-sm font-medium text-foreground"
            >
