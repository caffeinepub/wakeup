...
import { useGetMyEmployerProfile, useGetJobPostingById } from '../hooks/useQueries';
...
export default function JobPostingFormPage({ mode }: Props) {
  ...
  const { data: employer, isLoading: employerLoading } = useGetMyEmployerProfile();
  const { data: jobPosting, isLoading: jobLoading } = useGetJobPostingById(isEdit ? jobId : null);
      ...
      <div className="container mx-auto px-4 py-8 max-w-2xl">
        <div className="mb-6">
          ...
          <p className="text-muted-foreground text-sm mt-1">
            {isEdit ? 'Update the details of your job posting.' : 'Fill in the details to post a new pharmacy job.'}
          </p>
        </div>
