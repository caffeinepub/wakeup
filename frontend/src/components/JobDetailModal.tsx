...
import { useGetEmployerById, useGetCallerUserProfile, useGetMyProfile, useApplyToJob } from '../hooks/useQueries';
...
import {
  MapPin, Briefcase, DollarSign, Building2, Globe,
  Users, Clock, GraduationCap, FileText, CheckCircle2, Loader2, Send
} from 'lucide-react';
...
export default function JobDetailModal({ job, onClose }: Props) {
  const { data: employer, isLoading: employerLoading } = useGetEmployerById(job.employerId);
  ...
  const specializations = job.specializations.split(',').map(s => s.trim()).filter(Boolean);
  const qualifications = job.qualifications.split(',').map(s => s.trim()).filter(Boolean);
      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-start gap-3">
            <div className="w-12 h-12 rounded-xl bg-secondary flex items-center justify-center text-primary font-display font-bold text-xl shrink-0">
              {employer?.companyName?.charAt(0)?.toUpperCase() ?? <Building2 className="w-6 h-6" />}
            </div>
            <div className="flex-1 min-w-0">
              <DialogTitle className="font-display text-xl leading-tight">{job.title}</DialogTitle>
        ...
        <div className="space-y-4 mt-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            <div className="bg-secondary rounded-lg p-3 space-y-1">
              <p className="text-xs text-muted-foreground uppercase tracking-wide font-medium">Salary Range</p>
              <p className="text-sm font-medium text-foreground flex items-center gap-1">
          ...
          <div className="space-y-2">
            <div className="flex flex-wrap gap-1.5">
              {specializations.map(s => (
                <Badge key={s} variant="secondary" className="text-primary">{s}</Badge>
              ))}
              {qualifications.map(q => (
                <Badge key={q} variant="outline" className="flex items-center gap-1">
                  <GraduationCap className="w-3 h-3" />{q}
                </Badge>
              ))}
            </div>
          </div>
          ...
          {employer && (
            <>
              ...
              <div className="space-y-2">
                <h4 className="text-sm font-semibold text-foreground flex items-center gap-1.5">
                  <Building2 className="w-4 h-4 text-primary" /> About {employer.companyName}
                </h4>
                <div className="flex flex-wrap gap-3 text-xs text-muted-foreground mb-2">
                  <span className="flex items-center gap-1">
                    <Users className="w-3.5 h-3.5" />{employer.companySize} employees
                  </span>
                  <span className="flex items-center gap-1">
                ...
                {employer.about && (
                  <p className="text-sm text-muted-foreground leading-relaxed">{employer.about}</p>
                )}
              </div>
            </>
          )}
