...
import { MapPin, Briefcase, DollarSign, Building2, ChevronRight, Clock } from 'lucide-react';
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-start gap-3 mb-2">
              <div className="w-10 h-10 rounded-lg bg-secondary flex items-center justify-center text-primary font-display font-bold shrink-0">
                {employer?.companyName?.charAt(0)?.toUpperCase() ?? <Building2 className="w-5 h-5" />}
              </div>
            ...
            <div className="flex flex-wrap gap-3 text-xs text-muted-foreground mb-3">
              ...
              <span className="flex items-center gap-1">
                <Clock className="w-3.5 h-3.5" />{postedLabel}
              </span>
            </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {specializations.map(s => (
                <Badge key={s} variant="secondary" className="text-primary text-xs">{s}</Badge>
              ))}
